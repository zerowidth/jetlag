...
describe "AR with Time.zone_default set" do
  ...
  context "and default_timezone = :local" do
    ...
    context "and time_zone_aware_attributes = true (default)" do
      ...
      context "with Time.zone set to US/Eastern" do
        before :each do
        ...
        it "reads the timestamp in the configured local timezone (ok)" do
          item = @items.create :ts => @time
          ts = item.reload.ts
          ts.inspect.should == "Tue, 12 Apr 2011 13:30:00 EDT -04:00"
        end
...
describe "AR with Time.zone_default set to Eastern (db is Mountain) and Time.zone set to Eastern" do

  before :all do
    ENV["TZ"].should == "US/Mountain" # sanity
    Time.zone_default = Time.__send__(:get_zone, "Eastern Time (US & Canada)")
    ActiveRecord::Base.default_timezone = :utc # default
    ActiveRecord::Base.time_zone_aware_attributes = true # default
  end
